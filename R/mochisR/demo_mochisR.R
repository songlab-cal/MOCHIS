...
# 1. Quadratic kernel
mochis.test(x=x3,y=y3,p=1,wList=length(x3):0,approx="resample",use_base_r=FALSE)
quad_w_vec_3 <- sapply(1:(length(x3)+1), function(x) {(x/(length(x3)+1)-0.5)^2})
mochis.test(x=x3,y=y3,p=1,wList=quad_w_vec_3,approx="resample")
# 2. Gaussian kernel
gauss_w_vec_3 <- sapply(1:(length(x3)+1), function(x) {qnorm(x/(length(x3)+1.5))^2})
mochis.test(x=x3,y=y3,p=1,wList=gauss_w_vec_3,approx="resample")
...
mochis.test(x=x4,y=y4,p=1,wList=length(x4):0,approx="resample",use_base_r=FALSE)
