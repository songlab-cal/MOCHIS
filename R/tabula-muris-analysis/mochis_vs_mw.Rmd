...
4. There are $2174$ persistently DEGs, of which $41$ are _entirely_ missed by Mann-Whitney test.
5. For the $41$ persistently DEGs detected by MOCHIS, we find heterogeneous patterns of change in dispersion over time. In particular, it is _not always true_ that the change in dispersion is unidirectional (usually thought to be positive as an organism ages).